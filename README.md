...
## Round 4 Research Pipeline

When `TURSO_DATABASE_URL` and `TURSO_AUTH_TOKEN` are configured, Turso is the
authoritative store for the Round 4 tables, including `watchlist`,
`holdings_daily`, `job_log`, `quote_daily`, `hypotheses`, and `briefs`. The
Longbridge migration is idempotent and its state is reported by `/health`.
...
records each stock result in `job_log`. Inspect it with
`GET /admin/jobs/{job_id}`. A successful worker also writes the numeric brief
to Turso; no Chinese interpretation is generated by the API.

Research endpoints are authenticated: `GET /timeline?code=06182`,
`GET /panel/broker_daily?group=lshape79&format=csv`,
`GET /panel/transfers?group=lshape79`, `GET /brief/latest`,
`GET /brief/YYYY-MM-DD`, `POST /admin/hypotheses`, and
`GET /hypotheses?status=pending`. CSV panel responses are streamed. The
Streamlit Research Panel reads the same persisted tables and shows the daily
brief, timeline, transfer candidates, and broker holding stack.
